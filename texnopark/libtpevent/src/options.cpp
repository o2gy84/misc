...
namespace 
{
    ...
    void usage(const std::string &progname)
    {
        ...
        ss << std::endl << "\tUsage: " 
            << progname << " "
            << "--" << opt_port << "=port "
            << "--" << opt_engine << "=[select|poll|epoll] "
            << "--" << opt_async << "=[0|1]"
            << std::endl << std::endl;

        ss  << "\t" << opt_port << ": 1025..65536" << std::endl
            << "\t" << opt_engine << ": engine for event loop" << std::endl
            << "\t" << opt_async << ": sync or async (threaded) engine" << std::endl << std::endl;
    ...
    {
        if (s == "select") return engine_t::SELECT;
        if (s == "poll") return engine_t::POLL;
...
Options::Options(int count, const char *const *args)
{
    ...
    for (int i = 1; i < count; ++i)
    {
        ...
        else if (opt_name == opt_async)
        {
            ...
            if (async == 0) m_Async = false;
            else if (async == 1) m_Async = true;
            else
            {
                usage(args[0]);
                throw std::runtime_error("bad options");
